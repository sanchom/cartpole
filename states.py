...
class StateBinner(object):
  def _bin_value(self, value, low, high, bins):
    if value < low:
      return 0
    elif value > high:
      return bins + 1
  ...
  def discretize(self, observation):
    return tuple([self._bin_value(v, low, high, n) for (v, (low, high), n) in zip(observation, self.state_bounds, self.state_bins)])


# State-space taken from Matthew Chan
# (https://medium.com/@tuzzer/cart-pole-balancing-with-q-learning-b54c6068d947)
class CartpoleStateBinner(StateBinner):
...
class AcrobotStateBinner(StateBinner):
  def __init__(self, env):
    self.state_bounds = list(zip(env.observation_space.low, env.observation_space.high))
